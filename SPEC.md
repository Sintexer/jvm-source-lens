...
**`get_class_structure`:** optional **`scope`**: `overview` (default compact), `declared`, `effective`. Method lines look like source declarations, e.g. `calculate(long price, long quantity)`. Inherited methods capped in `effective` with a plain-text footer. **`full: true`** → JSON payload projected by `scope` (same as compact detail levels). Optional **`include`** expands projections: `signatures` / `inherited` (method objects with full signatures; also implied by `scope=declared/effective`), `fields`, `hierarchy`, `annotations`, `provenance` (full provenance object instead of slim), `all` (all sections). Default JSON includes `declaredMethodNames[]` (names only), slim `provenance` (`libName` + `kind`), no method objects.
...
**`get_method_signature`:** default `full=true` JSON per overload: `declarationLine` only. Optional **`include`**: `parameters` → `visibility`, `returnTypeDisplay`, `parameters[]`; `exceptions` → `thrownExceptions[]`; `jvmDescriptor` → JVM bytecode fields; `provenance` → full provenance object; `all` → all fields. Provenance default: slim `libName` + `kind`.

**Cross-cutting provenance projection:** `get_class_structure`, `get_method_signature`, and `find_in_class_source` default to slim provenance (`libName` + `kind`) in `full=true` JSON. Pass `include: ['provenance']` for the full object with `coordinates` and `jarPath`/`moduleRoot`.
