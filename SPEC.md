...
```bash
# Omitting --module: auto-picks the unique module that owns the FQN
jvmsrc get com.example.MyClass --project /path/to/project

# Explicit submodule scope (required when several modules own the type)
jvmsrc get com.example.MyClass --project /path/to/project --module :core:utils
```

When **`--module` / `modulePath` is omitted** on a multimodule project, class-scoped tools (**`get`**, **`get_class_source`**, **`get_class_structure`**, **`get_method_signature`**, **`find_in_class_source`**) probe every module in **`ResolutionOutput`** for the FQN. A **unique** owner is used transparently. **Multiple** owners return **`MODULE_AMBIGUOUS`** with candidate module paths (never a silent pick). **No** owner returns **`CLASS_NOT_FOUND`**, enriched with **`suggestedModulePaths`** (retry with a leaf consumer module) and optional simple-name **`suggestions`** ("did you mean"). Explicit **`modulePath`** is unchanged.

**Config-scoped tools** (**`search_classes`**, and any caller of **`pickResolvedConfiguration`** without **`modulePath`**): prefer a module named **`root`** when it has the wanted configuration; otherwise uniquely pick the sole non-root module that has it; several → **`MODULE_AMBIGUOUS`** with **`modulePaths`**; none → **`CONFIGURATION_NOT_FOUND`**. **`MODULE_NOT_FOUND`** / **`CONFIGURATION_NOT_FOUND`** include optional **`availableModules[]`** and list them in **`message`** so agents can retry without a separate **`resolve_dependencies`** call.
...
**Stable `code` values** on failures: **`RESOLUTION_FAILED`**, **`SOURCES_RESOLVE_FAILED`**, **`INVALID_FQN`**, **`MODULE_NOT_FOUND`**, **`CONFIGURATION_NOT_FOUND`**, **`MODULE_AMBIGUOUS`**, **`ZIP_READ_ERROR`**, **`CLASS_NOT_FOUND`**, **`DECOMPILE_FAILED`**, **`SIGNATURE_EXTRACT_FAILED`**, **`EXCERPT_REQUEST_INVALID`**, **`EXCERPT_NOT_FOUND`**, **`FIND_QUERY_INVALID`**, **`FIND_SOURCE_TOO_LARGE`**, **`SOURCE_OUTPUT_TOO_LARGE`**. **`SIGNATURE_EXTRACT_FAILED`** covers **javap** failures for **`get_method_signature`** (per-method) and **`get_class_structure`** (whole-class disassembly; `methodName` may be omitted in the error object). Excerpt failures apply when **`get_class_source`** / CLI **`get`** is called with **`methodNames`** / **`methodName`** and/or **`startLine`** / **`endLine`**. Find-in-source failures apply to **`find_in_class_source`** / CLI **`find-in-class`**. **`SOURCE_OUTPUT_TOO_LARGE`** applies when **`get_class_source`** / CLI **`get`** is asked for **unscoped full source** larger than **`JVMSRC_MAX_FULL_SOURCE_CHARS`** (default 64 KiB); agents must pass **`methodNames`** or a line range (or raise the env). Soft truncation via **`JVMSRC_MAX_SOURCE_OUTPUT_CHARS`** still applies to excerpted responses.

**Module-path auto-infer and did-you-mean (class-scoped tools):** When **`modulePath`** is omitted on **`get_class_source`**, **`get_class_structure`**, **`get_method_signature`**, and **`find_in_class_source`**, the tool probes every module in **`ResolutionOutput.modules[]`** for the class (via the same classpath search used for a normal lookup) instead of silently defaulting to the root module. A **unique** owning module is used transparently (no extra round trip). **Multiple** owning modules return **`MODULE_AMBIGUOUS`** (**`modulePaths`** lists every candidate; retry with one of them). **No** owning module falls through to **`CLASS_NOT_FOUND`**, enriched with **`suggestedModulePaths`** (every module name, so the agent can retry scoped to a specific module) and, when a class-search index is available for the searched module(s), **`suggestions`** — up to 5 case-insensitive exact simple-name matches (excluding the exact miss) ranked the same way as **`search_classes`**, for a "did you mean" nudge. Both fields are optional and omitted when empty; an explicit **`modulePath`** always skips probing and is used as-is (no inference, no ambiguity).

**Config-scoped module pick (`search_classes`):** When **`modulePath`** is omitted, **`pickResolvedConfiguration`** prefers **`root`** if it has the wanted config; else the unique leaf module that has it; else **`MODULE_AMBIGUOUS`** / **`CONFIGURATION_NOT_FOUND`** with **`availableModules`**.
...
Default stderr failure JSON is terminal-oriented: multiline `message` text is flattened to one line, and large nested fields (for example raw Gradle `stderr`) are omitted from that line to avoid noisy escaped `\\n` output. Full structured failure details remain available via **`--json`** and diagnostics records (§6.3).
...
**`get_class_structure`:** optional **`scope`**: `overview` (default compact), `declared`, `effective`. Compact method/field lines use abbreviated modifiers (**`P`/`p`/`prot`/`pack`** + **`s`/`a`/`f`**, e.g. `Psf long ZERO`, `Ps void foo()`) — legend also in MCP tool descriptions and server instructions. **`overview`** is method-first, but when the type is **field-heavy** (≥8 fields and ≤2 non-constructor declared methods) it lists fields (capped at 40) instead of only a count — same heuristic in **`full: true`** overview JSON. Inherited methods capped in `effective` with a plain-text footer. **`full: true`** → JSON payload projected by `scope` (same as compact detail levels; JSON keeps full `visibility` / `static` / `final`). Optional **`include`** expands projections: `signatures` / `inherited` (method objects with full signatures; also implied by `scope=declared/effective`), `fields`, `hierarchy`, `annotations`, `provenance` (full provenance object instead of slim), `all` (all sections). Default JSON includes `declaredMethodNames[]` (names only), slim `provenance` (`libName` + `kind`), no method objects (unless field-heavy overview includes capped **`fields[]`**).

**`search_classes`:** default hits are **`className`** (FQN) and **`libName`** (Maven artifactId or Gradle module) — a suggestion list, not full jar paths. Optional MCP **`include`** expands hits and/or attaches **`indexMeta`** (`simpleName`, `score`, `origin`, `coordinates`, `location`, `scope`, `indexMeta`, `all`). Same projection in compact text and **`full: true`** JSON. Non-glob **`query`**: whitespace-separated tokens are **AND**'d as independent case-insensitive substrings (not one contiguous phrase).

**`resolve_dependencies`:** default `full=true` JSON is a **summary** (no artifact lists): `schemaVersion`, `resolvedAt`, `buildSystem`, `projectRoot`, `moduleCount`, per-module config counts, `errors[]`. Optional **`include`** expands: `artifacts` / `coordinates` → per-config `artifacts[]` with `group/name/version/origin/direct`; `jarPaths` → adds `jarPath/sourcesJarPath` to artifacts (requires `artifacts`); `all` → full `ResolutionOutput` (current pre-projection behavior).

**`find_in_class_source`:** default `full=true` JSON per hit: `line`, `column`, `matchedText` only. Optional **`include`**: `context` → `contextBefore[]` / `contextAfter[]`; `block` → `block { startLine, endLine }`; `provenance` → full provenance object; `all` → all fields.

**`get_method_signature`:** compact overload lines use the same modifier abbreviations as **`get_class_structure`**. Default `full=true` JSON per overload: `declarationLine` only (full Java spelling). Optional **`include`**: `parameters` → `visibility`, `returnTypeDisplay`, `parameters[]`; `exceptions` → `thrownExceptions[]`; `jvmDescriptor` → JVM bytecode fields; `provenance` → full provenance object; `all` → all fields. Provenance default: slim `libName` + `kind`. When the class exists but no overloads match **`methodName`**: **`found: false`**, **`methodFound: false`**, **`querySucceeded: true`**, **`isError: false`** (aligned with search/find empty hits — the name may be a field).

**Cross-cutting provenance projection:** `get_class_structure`, `get_method_signature`, and `find_in_class_source` default to slim provenance (`libName` + `kind`) in `full=true` JSON. Pass `include: ['provenance']` for the full object with `coordinates` and `jarPath`/`moduleRoot`. Compact text provenance footers omit absolute filesystem paths (coordinates + kind / module name only).
...
|---|---|---|
| `find_in_class_source` | **Implemented** | Resolves source for **`className`** (same classpath path as **`get_class_source`**), then searches for **`query`** (literal substring by default; optional **`regex: true`**). Optional **`contextLines`** (default 3, max 50), **`maxHits`** (default 20, max 100). **Found:** **`isError: false`**, **`found: true`**, **`hits[]`** with **`line`**, **`column`**, **`matchedText`** (default compact and `full=true`), optional **`block`** / **`contextBefore`** / **`contextAfter`** (opt-in via **`include`**: `context`, `block`), **`provenance`** (slim `libName`+`kind` by default; opt-in `provenance`/`all` for full object), **`lineNumbersReliable`** (false when decompiled). **No matches (class resolved):** **`found: false`**, **`querySucceeded: true`**. **Class absent:** same as **`get_class_source`** (**`CLASS_NOT_FOUND`**). CLI: **`jvmsrc find-in-class <className> <query>`**. |
| `get_class_source` | **Implemented** | Returns Java source (original **`interproject`** / **sources JAR** or CFR-decompiled bytecode) for a **fully-qualified** class name. Tool arguments: **`className`**, **`projectRoot`**, optional **`modulePath`**, **`configuration`**, **`includeTest`**, **`forceRefresh`** (same semantics as CLI `get`). Optional excerpt: **`methodNames`** (array; use **`<init>`** for constructors; all overloads included) and/or **`methodName`** (merged into **`methodNames`**) and/or **`startLine`** / **`endLine`** (1-based inclusive; both required for line slices). Omit excerpt params for the full compilation unit (refused with **`SOURCE_OUTPUT_TOO_LARGE`** when over **`JVMSRC_MAX_FULL_SOURCE_CHARS`**, default 64 KiB — stderr **`[jvmsrc] FATAL`** + file diagnostic). **Found:** **`isError: false`**, **`found: true`**, **`source`**, **`sourceAvailable`**, **`provenance`**, optional **`excerpt`** metadata (`matchedMethodNames`, `unmatchedMethodNames`, `lineNumbersReliable` — **`false`** when **`sourceAvailable: false`**). When one or more requested **`methodNames`** are absent from the class's own compilation unit, jvmsrc walks superclasses/interfaces on the classpath (same hierarchy BFS and visit cap as **`get_class_structure`**; sources JAR / inter-project preferred, bytecode-only or JDK supertypes are traversed for header only and skipped for body extraction) before giving up; bodies found this way are appended to **`source`** behind a `// declaringClass: <fqn>` comment and listed in **`excerpt.inheritedExcerpts[]`** (`methodName`, `declaringClass`), and count toward **`matchedMethodNames`**. If nothing matches anywhere, **`EXCERPT_NOT_FOUND`**'s **`message`** states that superclasses/interfaces were also searched. **Not on classpath (successful scan):** **`isError: false`**, **`found: false`**, **`querySucceeded: true`** — do not retry as a transient failure. **Failures:** **`isError: true`** with **`errorCategory`**, **`isRetryable`**, **`message`** (agent-directed next step), stable **`code`** (§7), and domain **`error`** (including **`EXCERPT_*`** for bad or unmatched excerpt requests). **Happy-path success** (`found: true`, data returned): **`structuredContent`** when **`full: true`** includes **`found`**, **`querySucceeded`**, **`errorCategory: null`** only (no **`message`**); compact text is payload only (no **`---`** footer). **Empty results** (e.g. **`CLASS_NOT_FOUND`**, **`search_classes`** zero hits, **`find_in_class_source`** no match, **`methodFound: false`**) include **`message`** plus the same outcome fields; compact mode appends them in a **`---`** footer. |
| `get_method_signature` | **Implemented** | **Overload listing** (§7.2). **Default:** **source-first** classpath **`.java`** when parseable; else **`javap -private -verbose`**. Optional **`bytecodeOnly: true`**: **javap only** (no sources/**`src/`** fallback); **`sourceAvailable`** always **`false`**. Arguments: **`className`**, **`methodName`**, **`projectRoot`**, optional **`modulePath`**, **`configuration`**, **`includeTest`**, **`forceRefresh`**, **`bytecodeOnly`**. Optional **`include`**: `parameters`, `exceptions`, `jvmDescriptor`, `provenance`, `all` — expand `full=true` JSON per overload beyond default `declarationLine`-only. **`JVMSRC_JAVAP_*`** env (§6.2). **Class present, no matching overloads:** **`isError: false`**, **`found: false`**, **`methodFound: false`**, **`querySucceeded: true`**. **Class absent:** **`found: false`** with **`CLASS_NOT_FOUND`**. |
| `get_class_structure` | **Implemented** | **IDE-shaped API browse** (§7.2). **Default compact:** plain text; optional **`scope`**: `overview` \| `declared` \| `effective` (declaration-line method lists). Field-heavy overview lists fields (see §8.1.4). **`full: true`:** JSON projected by `scope` — default `overview` shape: `declaredMethodNames[]`, slim `provenance` (`libName`+`kind`), no method objects (capped **`fields[]`** when field-heavy). Optional **`include`**: `signatures` / `inherited` (full method objects; implied by `scope=declared/effective`), `fields`, `hierarchy`, `annotations`, `provenance` (full provenance), `all`. Does not run CFR. **`sourceAvailable`** when primary type came from original source. |
| `search_classes` | **Implemented** (index v3) | **Classpath discovery** when the FQN is unknown (§12.3). Arguments: **`query`** (non-empty), **`projectRoot`**, optional **`modulePath`**, **`configuration`**, **`includeTest`**, **`forceRefresh`**, **`limit`** (default 50, max 200), optional **`include`** (response projection; see §8.1.4). Resolves or loads cached Gradle output, builds or reuses **`class-search-index.json`** in the resolution cache bucket (with **`jar-fqn-cache.json`** stat-based reuse for unchanged JAR paths). **Default hits:** **`className`**, **`libName`** (artifactId for external JARs; inter-project module name for **`interproject`**). Optional **`include`:** **`simpleName`**, **`score`**, **`origin`**, **`coordinates`**, **`location`** (`jarPath` / `moduleRoot` / `interprojectModuleName`), **`scope`** (`moduleName`, `configurationName`), **`indexMeta`** (index build stats at payload root), **`all`** (full per-hit fields plus **`indexMeta`**). **Substring query:** case-insensitive; whitespace-separated tokens are **AND**'d over index **`searchText`** (FQN, simple name, plus source enrichment when available). **Glob query:** **`*`** / **`?`** apply to **FQN and simple name only** (no AND split). **Index:** external and **`local-file`** JAR **`.class`** names (ZIP central directory) plus source enrichment as above; inter-project **`src/main/java`** and **`src/test/java`** when **`includeTest`**. **Module omit:** config-scoped auto-pick (§5.4 / §7). **Success:** **`isError: false`**, **`ok: true`**, **`found`** (`false` when **`totalMatches === 0`**), **`querySucceeded: true`**, **`totalMatches`**, **`hitCount`**, **`hits[]`**; **`indexMeta`** only when requested via **`include`**. When **`totalMatches === 0`**, also **`message`** (agent-directed; multi-word queries explain AND semantics). **Failures:** same envelope as **`resolve_dependencies`** or validation (**`MODULE_NOT_FOUND`**, **`CONFIGURATION_NOT_FOUND`**, **`MODULE_AMBIGUOUS`**). |
| `resolve_dependencies` | **Implemented** | Returns dependency graph for the whole project (all `modules[]`). Tool arguments: **`projectRoot`**, optional **`forceRefresh`** (same semantics as CLI `resolve` / §6.1), **`include`** (response projection). **Default `full=true` JSON:** summary — `schemaVersion`, `resolvedAt`, `buildSystem`, `projectRoot`, `moduleCount`, per-module config counts, `errors[]`. Optional **`include`**: `artifacts`/`coordinates` → per-config `artifacts[]`; `jarPaths` → artifact paths; `all` → full `ResolutionOutput`. **Compact text:** unchanged module/config count summary. **Failures:** **`isError: true`** with **`errorCategory`**, **`isRetryable`**, **`message`**, **`code: RESOLUTION_FAILED`**, and domain **`error`**. Use before batch **`get_class_source`** calls to warm the resolution cache without per-class Gradle runs. |
...
**MCP error categories (`get_class_source`, **`get_class_structure`**, **`get_method_signature`**, **`resolve_dependencies`**, **`search_classes`**, **`find_in_class_source`**):** Tool failures set **`isError: true`** and include **`errorCategory`**, **`isRetryable`**, and **`message`** (agent instruction with the next tool call to try). **`transient`** — Gradle/network/timeouts, CFR timeouts, **javap** timeouts / spawn issues; retry after a delay. **`validation`** — bad `projectRoot`, FQN, **`methodName`**, `modulePath`, or `configuration`, or **`SOURCE_OUTPUT_TOO_LARGE`**; fix inputs. **`business`** — e.g. CFR cannot decompile, sources permanently unavailable, **javap** exited non-zero without transient hints; do not retry the same request. **`permission`** — repository authentication denied **or** missing repository credentials / unset credential environment variables required by the project’s Gradle scripts (not only HTTP 401/403); fix auth or set those vars on the **jvmsrc process** (MCP hosts often do not inherit interactive shell env), then retry. A class missing after a **successful** classpath scan is **`found: false`** with **`isError: false`** (not confused with “could not reach Gradle”). For **`get_method_signature`**, when the class is found but **`methodFound: false`**, the tool returns **`isError: false`**, **`found: false`** — adjust **`methodName`** (constructors **`<init>`**), or use **`get_class_structure`** if the name is a field/constant.
