...
| `search_classes` | **Implemented** (v1) | **Classpath discovery** when the FQN is unknown (§12.3). Arguments: **`query`** (non-empty), **`projectRoot`**, optional **`modulePath`**, **`configuration`**, **`includeTest`**, **`forceRefresh`**, **`limit`** (default 50, max 200). Resolves or loads cached Gradle output, builds or reuses a disk index under the resolution cache bucket, returns ranked hits: **`className`**, **`simpleName`**, **`moduleName`**, **`configurationName`**, **`origin`** (`external` \| `interproject`), **`coordinates`**, **`jarPath`** / **`moduleRoot`** / **`interprojectModuleName`**, **`score`**. **Query:** case-insensitive substring over FQN + simple name, or glob with **`*`** / **`?`** on FQN or simple name. **v1 index:** external JAR **`.class`** names (ZIP central directory only, no inflate) plus inter-project **`src/main/java`** and **`src/test/java`** when **`includeTest`**. **`origin: local-file`** artifacts are skipped (**`indexMeta.skippedArtifacts`**). **Success:** **`isError: false`**, **`ok: true`**, **`querySucceeded: true`**, **`totalMatches`**, **`hitCount`**, **`hits[]`**, **`indexMeta`**. **Failures:** same envelope as **`resolve_dependencies`** or validation (**`MODULE_NOT_FOUND`**, **`CONFIGURATION_NOT_FOUND`**). Method-name / Javadoc full-text enrichment is future work (§12.2). |
...
| Medium | **`search_classes`** / class search index (capability discovery; index-backed) | §12 — **implemented** (§8.2 v1: class names + globs; method/Javadoc index deferred) |
